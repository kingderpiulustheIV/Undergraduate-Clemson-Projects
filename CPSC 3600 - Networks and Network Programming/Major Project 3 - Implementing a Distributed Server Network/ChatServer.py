...
# 12/8/2024
# CPSC 3600 Robb
# Major Project - Implementing a Distributed Server Network
...
##############################################################################################################
    ...
    def setup_server_socket(self):
        ...
        # if the port is already in use, print an error message and return
        except error as e:
            self.print_info("Error binding to port %i: %s" % (self.port, e))
            return
    ...
    def connect_to_server(self):
        ...
        self.connect_socket = socket(AF_INET, SOCK_STREAM)
        self.connect_socket.connect((self.connect_to_host_addr, self.connect_to_port))
        ...
        # create a ServerRegistrationMessage and send it to the client
        data.write_buffer = b''
        events = selectors.EVENT_READ | selectors.EVENT_WRITE
        reg_message = ServerRegistrationMessage.bytes(self.id, 0, self.server_name, self.server_info)
        data.write_buffer += reg_message
        self.sel.register(self.connect_socket, events, data)
    ...
    def check_IO_devices_for_messages(self):
        ...
        # TODO: Implement the above functionality
        # While loop to check for IO devices
    ...
    def cleanup(self):
        ...
        # TODO: Implement the above functionality
        # finds resources to clean
        devices = list(self.sel._fd_to_key.values())

        # Loops and closes and cleans all resources
        for device in devices:
            device.fileobj.close()
            self.sel.unregister(device.fileobj)
        self.sel.close()
    ...
    def accept_new_connection(self, io_device):
        ...
        # sets the connection to accept an object
        conn, addr = io_device.fileobj.accept()
        conn.setblocking(False)

        # Retrieves data from the new connection
        selector_data = BaseConnectionData()
        selector_data.write_buffer = b''
        events = selectors.EVENT_READ | selectors.EVENT_WRITE
        self.sel.register(conn, events, selector_data)
    ...
    def handle_io_device_events(self, io_device, event_mask):
        ...
        """
        # TODO: Implement the above functionality
        #  Handles io in case of Reading mode.
        if event_mask & selectors.EVENT_READ:
            recv_data = io_device.fileobj.recv(1024)
            if recv_data:
        ...
        #  Handles io in case of Writing mode.
        if event_mask & selectors.EVENT_WRITE:
            wb = io_device.data.write_buffer
            if wb:
                io_device.fileobj.send(wb)
...
##############################################################################################################
    ...
    def send_message_to_host(self, destination_id, message):
        ...
        if destination_id in self.hosts_db:
            ...
            # TODO: Implement the above functionality
            # Appends the message to the write buffer
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        ...
        """
        # TODO: Implement the above functionality
        # Loops through all adjacent servers and sends the message to them
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        ...
        """
        # TODO: Implement the above functionality
        # Loops through all adjacent clients and sends the message to them
...
##############################################################################################################
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        """
        # TODO: Implement the functionality described above
        # Checks if the server ID already exists
        if message.source_id in self.hosts_db:
            ...
            return
        # Creates a new ServerConnectionData object
        ...
        # Sends a welcome status update to the newly connected adjacent server
        ...
        # Adds the new server to the adjacent_server_ids 
        if message.last_hop_id == self.id or message.last_hop_id == 0:
...
##############################################################################################################
    ...
    def handle_client_registration_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            ...
            io_device.data.write_buffer += status_message
            return
        # Creates a new ClientConnectionData object
...
##############################################################################################################
    ...
    def handle_status_message(self, io_device, message):
        ...
        """
        # TODO: Implement the functionality described above
        # appends the message to the status updates log if its self id or 0
        if message.destination_id == self.id or message.destination_id == 0:
            self.status_updates_log.append(message.content)
        # forwards the message to the intended destination.
...
##############################################################################################################
    ...
    def handle_client_chat_message(self, io_device, message):
        ...
        Returns:
            None        
        """
        # update the chat message if there is no destination id or id is unknown.
        # TODO: Implement the functionality described above
        if message.destination_id not in self.hosts_db:
            no_destination_message = StatusUpdateMessage.bytes(self.id, message.source_id, 0x01, f"Unknown ID {message.destination_id}")
            self.send_message_to_unknown_io_device(io_device, no_destination_message)
...
##############################################################################################################
    ...
    def handle_client_quit_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db:
            ...
            # Broadcasts the quit message to all adjacent servers and clients
