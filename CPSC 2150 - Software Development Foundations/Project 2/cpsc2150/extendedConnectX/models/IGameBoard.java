...
public interface IGameBoard
{
    ...
    public default boolean checkForWin(int c)
    {
        ...
        BoardPosition lastPlaced = new BoardPosition(rowPosition, c);

        char player = whatsAtPos(lastPlaced);

        // Now check for a win in every direction from the last placed token.
        if(checkVertWin(lastPlaced, player)) {
            return true;
        }
        else if(checkHorizWin(lastPlaced, player)) {
            return true;
        }
        else if(checkDiagWin(lastPlaced, player)) {
            return true;
        }
        else
        {
    ...
    public default boolean checkVertWin(BoardPosition pos, char p)
    {
        ...
        int numBelow = getNumRows() - pos.getRow();

        while(numBelow > 1)
        {
            numBelow = getNumRows() - pos.getRow();
